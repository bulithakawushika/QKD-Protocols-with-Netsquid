...
def run_e91():
    ...
    processorA = QuantumProcessor("procA", num_positions=20,
        phys_instructions=[
            PhysicalInstruction(INSTR_MEASURE, duration=3700),
            PhysicalInstruction(INSTR_MEASURE_X, duration=3700)
        ])
    processorB = QuantumProcessor("procB", num_positions=20,
        phys_instructions=[
            PhysicalInstruction(INSTR_MEASURE, duration=3700),
            PhysicalInstruction(INSTR_MEASURE_X, duration=3700)
        ])

    qchannel_alice = QuantumChannel("qchannel_alice", length=5,
    ...
    qchannel_bob = QuantumChannel("qchannel_bob", length=5,
        models={"delay_model": FibreDelayModel(c=2e5),
                "fibre_loss": FibreLossModel(p_loss_init=0.0, p_loss_length=0.0),
                "depolar_model": DepolarNoiseModel(depolar_rate=0.01)})
