...
class AliceProtocol(NodeProtocol):
    ...
    def run(self):
        ...
        while count < self.num_bits:
            yield self.await_port_input(self.node.ports["qout"])
            qubit = self.node.ports["qout"].rx_input().items[0]
            pos = self.processor.put(qubit)
            if pos is None:
                continue
            idx = pos[0]
            print(f"[Alice] Received qubit {count+1} in basis {self.bases[count]}")
            prog = QMeasure([self.bases[count]])
            yield self.processor.execute_program(prog, qubit_mapping=[idx])
            res = prog.output[str(idx)][0]
            self.results.append(res)
            count += 1
        print(f"[Alice] Results collected: {len(self.results)}")
